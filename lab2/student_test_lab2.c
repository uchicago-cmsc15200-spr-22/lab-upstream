...
int main(int argc, char **argv) {
    ...
    if (fabs(compute_div_fraction(4, 5) - 0.8) < EPSILON) {
        printf("Passed compute_div_fraction test\n");
    } else {
        printf("Failed compute_div_fraction test\n");
    }

    // Sample test for Exercise #2
    if (in_range_1(0.0, 1.0, 0.5, false)) {
        printf("Passed in_range_1 test\n");
    } else {
        printf("Failed in_range_1 test\n");
    }

    // Sample test for Exercise #3
    if (!in_range_2(0.0, 1.0, -0.5, true)) {
        printf("Passed in_range_2 test\n");
    } else {
        printf("Failed in_range_2 test\n");
    }

    // Sample test for Exercise #4
    if (clip(-0.5, 0.0, 4.0) == 0.0) {
        printf("Passed clip test\n");
    } else {
        printf("Failed clip test\n");
    }

    // Sample test for Exercise #5
    if (extract_flag(0x127, 7) == 0) {
        printf("Passed extract_flag test\n");
    } else {
        printf("Failed extract_flag test\n");
    }
